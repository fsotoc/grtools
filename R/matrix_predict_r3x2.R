matrix_predict_r3x2<-function(means,covmat,b,c){
  # computes a predicted matrix of confusion probabilities for the 2x2 design with
  # 2 bounds in the x-axis (for type-2 analysis of GRTapas),
  ...
  # b is a 3x2 matrix with one row for each vector of slope parameters
  # c is a column vector (3x1 matrix) with constants from decision bounds
  ...
  p_matrix <- matrix(0, nrow=4, ncol=6, byrow=T)
  ...
  for (i in 1:4){
    ...
    const3<-b[3,]%*%means[i,]+c[3,]
    
    p<-matrix(0,1,6)+.000001
    for(k in 1:100){
      z[1]<-z_vector[k]
      for(l in 1:100){
        z[2]<-z_vector[l]
        #apply all discriminant functions to z-vector
        h_1<-scal1%*%z+const1
        h_2<-scal2%*%z+const2
        h_3<-scal3%*%z+const3
        if (h_1 < 0){
