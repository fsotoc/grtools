...
grt_wind_VAR1_nll <- function(data,params){
  ...
  for (sub in 1:N){
    ...
    # get individual attention parameters
    kap = params[10+(sub-1)*6+1]
    lam1 = params[10+(sub-1)*6+2]
    lam2 = 1-lam1
    ...
    covmat[[2]] <- matrix(c(1/(kap*lam1), params[8]*(1/sqrt(lam1*lam2*kap^2)),
                            params[8]*(1/sqrt(lam1*lam2*kap^2)), 1/(kap*lam2)),
                          2,2,byrow=TRUE)
    
    covmat[[3]] <- matrix(c(1/(kap*lam1), params[9]*(1/sqrt(lam1*lam2*kap^2)),
                            params[9]*(1/sqrt(lam1*lam2*kap^2)), 1/(kap*lam2)),
                          2,2,byrow=TRUE)
    
    covmat[[4]] <- matrix(c(1/(kap*lam1), params[10]*(1/sqrt(lam1*lam2*kap^2)),
                            params[10]*(1/sqrt(lam1*lam2*kap^2)), 1/(kap*lam2)),
                          2,2,byrow=TRUE)
    
    # get decision bound parameters for this subject
    b <- matrix(c(1, params[10+(sub-1)*6+3], params[10+(sub-1)*6+5], 1),
                2,2,byrow=TRUE)
    c <- matrix(c(params[10+(sub-1)*6+4], params[10+(sub-1)*6+6]),2,1)
